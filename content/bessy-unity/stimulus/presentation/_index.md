...
Implementing `ISelectable` and `ITargetable`, any `StimulusPresentationBehaviour` must be able to indicate itself as the current training target and be selected *(though selection behaviour may be limited to indication, deferring logical selection to an unrelated component; see [Selection](../../selection/))*. Additionally, a presentation behaviour must implement methods to start and end stimulus presentation. These functional promises enable any extension of the base `StimulusPresentationBehaviour` class to be used interchangeably by other behaviours *(most prominently [Presenter Collections](../collections/))*.
